let store = {
  // нижнее подчеркивание значит то, что этот стейт нельзя юзать вне этого обьекта
  _state: {
    profilePage: {
      postsData: [
        {
  ...
  // нижнее подчеркивание значит то, что этот метод вызвается только внутри этого обьекта
  _callSubscriber() {
    console.log("State changed");
  },

  // чтобы юзать стетй нужно запускать этот метод который вернет стейт, котоырй можно перекидывать дальше
  // через пропсы
  getState() {
    return this._state;
  },
  ...
  dispatch(action) {
    if (action.type === "ADD-POST") {
      ...
      this._state.profilePage.postsData.push(res);

      this._callSubscriber(this._state);
      this._state.profilePage.textAreaData = "";
    } else if (action.type === "UPDATE-NEW-POST-TEXT") {
      this._state.profilePage.textAreaData = action.textData;
      this._callSubscriber(this._state);
    }
  },
};
